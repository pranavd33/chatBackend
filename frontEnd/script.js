...
const backendUrl = 'https://chatbackend-wf4x.onrender.com'; // We will get this URL in the next steps
...
async function selectUserToChat(user) {
  ...
  const response = await fetch('http://localhost:3000/chat/conversation/find-or-create', {
    method: 'POST',
...
async function fetchMessageHistory(conversationId) {
  ...
  const response = await fetch(`http://localhost:3000/chat/conversation/${conversationId}`);
...
UI.fileInput.addEventListener('change', async (event) => {
  ...
  const response = await fetch('http://localhost:3000/chat/upload', { method: 'POST', body: formData });
...
function addMessageToUI(sender, content, type) {
  ...
  } else if (content.startsWith('http://localhost:3000/uploads/')) {
  ...
  // This was the critical bug. It was trying to append messageWrapper to itself.
